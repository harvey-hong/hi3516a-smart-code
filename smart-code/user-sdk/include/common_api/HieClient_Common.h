...
using namespace ws_soft;  //hzg 20160218 下面不再需要重新定义ws_frame_type 和ws_frame_encode_type start end
...
typedef enum eStreamMediaType
{
	...
	eAssistOnlyVideo,					//2只传输子码流视频
	eAssistAndMainOnlyVideo,			//2同时请求主码流与子码流视频
...
typedef int(CALLBACK *CB_StreamMedia)(IN HSTREAM hStream, IN const StreamMediaFrame *cFrame, IN unsigned char *buf, DWORD channel);
...
*   接口类型: 阻塞式
*/
typedef int(CALLBACK *CB_StreamVoice)(IN HSTREAM hStream, IN const StreamMediaFrame *cFrame, IN unsigned char *buf, DWORD data);

...
/*
 * $Id: gdi.h,v 1.213 2005/09/04 06:31:18 weiym Exp $
 *
 ...
 *
 * This function saves the content of the main window \a hWnd to the iamge 
...
/**
 * \fn HICON GUIAPI CreateIcon (HDC hdc, int w, int h, 
 ...
 * \fn PLOGFONT GUIAPI CreateLogFont (const char* type, 
                ...
                char slant, char set_width, char spacing, char underline, 
...
/**
 * \fn wchar_t GUIAPI MB2WC (PLOGFONT log_font, 
                wchar_t *pwc, const unsigned char* mchar, int n)
 *
...
/**
 * \fn int GUIAPI MBS2WCS (PLOGFONT log_font, wchart_t* dest, 
 ...
 * \param n
 *
 * \return The number of wide characters converted.
 *
 * \sa WCS2MBS, mbstowcs
 */
 ...
 * \fn int GUIAPI WCS2MBS (PLOGFONT log_font, unsigned char* dest, 
                const unsigned short * wcs, int wcs_len, int n)
...
MG_EXPORT int GUIAPI WS_ICONV(ws_charset_type from, ws_charset_type to,const unsigned char* src,unsigned char *dest,size_t dest_size);
...
//转换图片格式，单状态图
MG_EXPORT int GUIAPI ChangeBitmapFormat_yuhj(const BITMAP *bg, BITMAP *icon, const RECT *rect);

//装换5状态图
MG_EXPORT int GUIAPI ChangeBitmapFormatEx_yuhj(const BITMAP *bg, BITMAP *icon, const RECT *rect, int state_num);

//将透明色修改为纯透明
MG_EXPORT int GUIAPI ChangeBitmapFormat_color_key_yuhj(BITMAP *icon, int transparent_color);

...
/*
 * $Id: edit.h,v 1.6 2005/04/26 09:26:51 snig Exp $
 *
...
/**
 * \def ES_ICON
 * \brief Specifies that the edit displays an icon.
 */
#define ES_ICON             0x00010000L

/**
 * \def ES_BITMAP
 * \brief Specifies that the edit displays a bitmap.
 */
#define ES_BITMAP           0x00020000L

#define ES_CONTENTMASK      0x000F0000L

#define ES_REALSIZEIMAGE    0x00000F00L
...
//add by zhangxinda
#if 1
#define EM_SETVALUELIMIT        (0xF0C5 + 0x0100) //set minValue and maxValue which edit want to limit the value
#define EM_SETSKBMODE           (0xF0C5 + 0x0101) //select the soft ketboard mode 
#define EM_CONNECT_CLOSE        (0xF0C5 + 0x0102) //when the num keyboard hide send the message to target_edit window
#define EM_ENABLEPINYIN           (0xF0C5 + 0x103)
#define EM_DISABLEPINYIN          (0xF0C5 + 0x104)
#endif 
...
#define EM_SET_BG_NORMAL          0xF0D0
#define EM_SET_BG_FOCUS          0xF0D1

#define EM_SETIMAGE_EXT		 	0xF0D3

#define EM_IMAGE_BITMAP         1
#define EM_IMAGE_ICON           2
...
/**
 *\def EM_SET_EDIT_TYPE
 *\brief set the edit type which is
 *	normal or ip or time
 *	int type;
 *	type = 0,normal edit;type = 1,ip edit;type = 2,time edit 
 *	wParam = type;
 *	lParam = 0;
 *	return 0;		add by ¡ı”Ó
 **/
#define EM_SET_EDIT_TYPE		0xF0E1

/**
 * \add by ¡ı”Ó
 * \def EM_SETSCOPETEXT
 *\brief set text form scope edit
 *SCOPE_TEXT scope_time
 *wParam = 0
 *lParam =(LRARAM)scope_time;
 *
 *
 *return 1;
 */
#define EM_SETSCOPETEXT			0xF0E2
#define EM_GETSCOPETEXT			0xF0E3
typedef struct _SSCOPE_TEXT{
	int start_hour;
	int start_min;
	int end_hour;
	int end_min;
...
typedef SSCOPE_TEXT* SCOPE_TEXT;
...
#define EN_MOUSE_ON	0x0800

    /** @} end of ctrl_edit_ncs */

	 /**
	 * \defgroup ctrl_edit_states States of edit control
	 * @{
	 */
/**
 * \def eST_UNCHECKED
 * \brief Indicates the edit is unchecked.
 */
#define EST_UNCHECKED       0x0001

/**
 * \def EST_FOCUS
 * \brief Specifies the focus state.
 */
#define EST_FOCUS           0x0002


/**
 * \def EST_INPUT
 * \brief Specifies in the input state.
 */
#define EST_INPUT           0x0004

/**
 * \def EST_INPUT
 * \brief Specifies in the checked state.
 */
#define EST_CHECKED         0x0008

		/** @} end of ctrl_edit_states */

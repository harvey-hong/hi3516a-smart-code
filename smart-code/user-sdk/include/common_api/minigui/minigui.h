...
/*
 * $Id: minigui.h,v 1.115 2005/09/04 05:07:28 weiym Exp $
 * 
 ...
 */
extern MG_EXPORT RECT g_rcScr;         /* The RECT of screen. */
 ...
 * And \a g_rcDesktop is defined as an alias (macro) of \a g_rcScr for 
 * MiniGUI-Threads.
 *
 * \sa g_rcScr
 */
extern MG_EXPORT RECT g_rcDesktop;     /* The RECT of desktop. */
 ...
 * \fn GHANDLE GUIAPI JoinLayer (const char* layer_name, 
                const char* client_name, int lx, int ty, int rx, int by)
 ...
 *
 * \param lx lx,ty,rx,by: The desktop rect will be returned through 
 *        these pointers.
 * \param ty lx,ty,rx,by: The desktop rect will be returned through 
 *        these pointers.
 * \param rx lx,ty,rx,by: The desktop rect will be returned through 
 *        these pointers.
 * \param by lx,ty,rx,by: The desktop rect will be returned through 
 *        these pointers.
 *
 * \sa JoinLayer
 */
...
/** 
 * \fn GHANDLE GUIAPI GetLayerInfo (const char* layer_name, RECT* max_rect, 
 ...
 *
 * \param layer_name The name of the layer.
 * \param max_rect The max desktop rect can be obtained will be returned 
...
/**
 * \var typedef void (* ON_CHANGE_LAYER) (int op, MG_Layer* layer, 
                MG_Client* client)
 ...
 * \brief Sets to a function to handle a comming in/going away connection of 
 * client.
 *
 ...
 * You can get the information of the client by accessing \a mgClients with 
 * \a cli.
 *
 ...
 *
 * The event will be passed through the argument of \a op, and the pointers 
 * to the relevant layer and client will be passed through the argument of 
 * \a layer and \a client respectively.
 *
 * \note Only available for the server of MiniGUI-Lite.
 * 
 * \sa ON_NEW_DEL_CLIENT, mgClients
 ...
 *
 * If the server want to disable output of clients, it can call 
 * this function to disable the clients' any drawing output.
 ...
 *       calling \a ClientCanDrawNowEx with \a bRepaint is TRUE and \a invrc 
 *       is NULL.
 *
 ...
 *
 * This function sets the topmost layer by the specified client identifier 
 * \a cli. It will bring the layer contains the client to be the topmost one.
 *
 ...
 * \brief Returns the client identifier from the process identifier (PID)
 *        of a client.
 *
 * This function gets the identifier of the sepcified client from the PID of it.
 *
 * \param pid The process identifier of the client.
...
MG_EXPORT int GUIAPI GetClientByPID (int pid);
     ...
     * i.e. \a mginit, of MiniGUI-Lite.
     *
     * A request consists of an identifier and the data associated with the 
     * request. The identifier is used by MiniGUI to determine which handler 
     * should be called when a request arrives. When MiniGUI finds one 
     * handler, it will call the handler and pass the socket fd connected to 
     * the client, the data associated with the request,
     ...
     * the server and wait for the reply. On the other hand, the request 
     * handler in the server will receive the request and call \a send_reply 
     * to send the reply to the client. In this way, you can create a simple 
     * IPC (inter-process conmmunication) mechanism between clients and 
     * the server.
     *
     * Example:
     *
 ...
 *
 * This function returns the file descriptor of the socket connected to 
 * the server, i.e. \a mginit.
 *
 ...
 * \param your_handler The handler of the request. Being NULL to 
 ...
 *
 * This function returns the request handler of the specified 
 * request identifier \a req_id.
 *
 ...
 *
 * After creating a listening socket by calling \a serv_listen, you can call 
 * this function to create a connection with a client. We also obtain the 
 * client's PID and UID from the pathname that it must bind before calling us.
 *
 * \param listenfd The fd of listen socket.
 * \param pidptr The client PID will be saved to this buffer when 
 *        this function returns.
 * \param uidptr The client UID will be saved to this buffer when 
 *        this function returns.
...
/**
 * \def MiniGUIMain (int args, const char* arg[])
 ...
 *
 * \note This function is available for MiniGUI-Lite and when 
 ...
 *
 * MiniGUI will try to use \a ./MiniGUI.cfg, \a ~/.MiniGUI.cfg, then \a 
 * /usr/local/etc/MiniGUI.cfg, and \a /etc/MiniGUI.cfg last.
 *
 ...
 * \retval ETC_SECTIONNOTFOUND  Can not find the specified section in 
 ...
 * \brief Creates a new clip board.
 *
 ...
 *
 * \retval CBERR_OK         The clip board has been created.
 ...
 * \brief Destroys a new clip board.
 *
 * This function destroys a clip board with the name \a cb_name.
 *
 * \param cb_name The name of the clip board.
 *
 * \retval CBERR_OK         The clip board has been destroyed.
 ...
 * \brief Sets the data of a clip board.
 *
 ...
 * \fn size_t GUIAPI GetClipBoardData (const char* cb_name, 
                void* data, size_t n);
 * \brief Gets the data of a clip board.
 *
 ...
 * \brief Gets a byte of from a clip board.
 *
 ...
 * You can use KS_* ORed with the status value to determine one shift key's s
 * tatus:
 *
 *  - KS_CAPSLOCK\n

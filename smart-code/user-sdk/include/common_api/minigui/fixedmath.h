...
/*
 * $Id: fixedmath.h,v 1.25 2005/09/04 05:18:26 weiym Exp $
 *
...
#define quat_interpolate(from, to, t, out)   \
                quat_slerp((from), (to), (t), (out), QUAT_SHORT)

static inline fixed dot_product (fixed x1, fixed y1, fixed z1, 
...
static inline float dot_product_f (float x1, float y1, float z1, 
...
#define CALC_ROW(n)     (fmul(x, m->v[n][0]) +        \
                         ...
                         fmul(z, m->v[n][2]) +        \
                         m->t[n])
static inline void apply_matrix (MATRIX *m, fixed x, fixed y, fixed z, 
...
static inline void persp_project (fixed x, fixed y, fixed z, 
...
static inline void persp_project_f (float x, float y, float z, 
